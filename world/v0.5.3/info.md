...
To generate new random world click `Generate World` button. Map seed value is a 8-digits lowercased hexadecimal number entered into prompt window. Only one pangea map type with 40x20 size is available for now. No lakes, no islands only one big continent with steppe and grassland tiles.
